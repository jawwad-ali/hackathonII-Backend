...
# This is critical because the OpenAI SDK reads OPENAI_API_KEY from os.environ directly
from dotenv import load_dotenv
load_dotenv()  # Must be called immediately, before any other imports
...
from src.config import settings, get_llm_circuit_breaker
...
app = FastAPI(
    ...
    description="Natural language interface for todo management using OpenAI Agents SDK",
    version="0.1.0",
    lifespan=lifespan,
)
...
async def health_check(response: Response):
    ...
    Returns:
        dict: HealthResponse schema with:
            ...
            - circuit_breakers: MCP and OpenAI circuit breaker states
    ...
    openai_breaker = get_llm_circuit_breaker()

    # Get circuit breaker states
    mcp_state = mcp_breaker.get_state()
    openai_state = openai_breaker.get_state()
    ...
    circuit_breakers = {
        ...
        "openai_api": breaker_state_to_dict(openai_state)
    }
    ...
    openai_open = openai_state.state.value == "open"

    if mcp_open and openai_open:
        status = "unhealthy"
    elif mcp_open or openai_open:
        status = "degraded"
    else:
        status = "healthy"
    ...
    # - HTTP 200 with status="degraded": MCP down but OpenAI available (graceful degradation)
    # - HTTP 200 with status="degraded": OpenAI down but MCP available (rare case)
    ...
    if status == "unhealthy":
        logger.warning(
            ...
            extra={
                ...
                "openai_state": openai_state.state.value
            }
        )
